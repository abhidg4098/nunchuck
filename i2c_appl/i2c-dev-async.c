...
static struct i2c_driver i2cdev_driver;
 ...
 int manage_rx(struct async_t *, int); /* requirement function */
...
static void async_routine(void *p) 
 {
   //struct  async_t *temp = (struct async_t*)p_instance;
   ...
   printk(KERN_ALERT "inside async_routine \n ");
   if(p_instance)
   printk(KERN_ALERT "p_client address = %d \n", p_instance->p_client.addr);
       //p_instance->p_fetched_data->byte = 0x07; // keep the data in kernel for now
       //p_instance->data_status = DATA_READY; // update the status of data

   //printk(KERN_ALERT "Returning back \n"); 
  // return;
   p_instance->ret_val = i2c_smbus_xfer(p_client.adapter,p_client.addr, p_client.flags,
                   data_arg.read_write, data_arg.command, data_arg.size,
                    p_instance->p_data);
    printk(KERN_ALERT "got the data!!! \n "); 
    if (!p_instance->ret_val && ((data_arg.size == I2C_SMBUS_PROC_CALL) ||
                 (data_arg.size == I2C_SMBUS_BLOCK_PROC_CALL) ||
                 (data_arg.read_write == I2C_SMBUS_READ))) 
     {
       printk(KERN_ALERT "successfully stored the data %d \n", p_instance->p_data->byte );
       p_instance->p_fetched_data = p_instance->p_data; // keep the data in kernel for now
       p_instance->data_status = DATA_READY; // update the status of data
     }
    else
     {
       printk(KERN_ALERT "Wrong call but still %d and returned %d \n", p_instance->p_data->byte, p_instance->ret_val );
 ...
 int manage_rx(struct async_t *params, int data_size)
 {
   int ret_val = 0;
   int ret_queue = -1;
   printk(KERN_ALERT "inside manage_rx with data_status = %d , client address = %d\n",p_instance->data_status, params->p_client.addr);
   if(DATA_READY == p_instance->data_status)
    {
     /*Data was ready and the work was complete, queue should be empty now */
      if (copy_to_user(p_instance->p_arg.data, p_instance->p_fetched_data, data_size))
      {
	printk(KERN_ALERT "fault copying to user! \n");
        return -EFAULT;
      }
      printk(KERN_ALERT "Success copy to user! \n");
   ...
   if(WORK_QUEUE_BUSY == p_instance->data_status)
   {
    ...
    copy_to_user(p_instance->p_data, &p_instance->data_status, 1);
    return -WORK_QUEUE_BUSY;
   }
  
  /*schedule a work in the queue */
  printk(KERN_ALERT "load the work to the queue status \n");
  ...
  /*scehduled a work, setting the queue as busy */ 
  printk(KERN_ALERT "returning from manage_rx ()! %d \n", ret_val);

  if(p_instance->p_data)
  printk(KERN_ALERT "User buffer value %d \n", p_instance->p_data->byte);
                ...
                unsigned long arg)
{
     ...
     //abhi
      ...
      p_instance->p_arg.data = data_arg.data; //from user
      p_instance->ret_val = 0;
      p_instance->p_data = &temp; //local address to hold the data
      //implementing the work queue
      printk(KERN_ALERT "before copy client address = %d and actual = %d \n",p_instance->p_client.addr,  client->addr );
      printk(KERN_ALERT "data_arg size my = %d, actual = %d \n",p_instance->p_arg.size, data_arg.size);
      res = manage_rx(p_instance, datasize); //(func_params, user_buff, size)
        ...
        case I2C_SLAVE_FORCE:
                ...
                if ((arg > 0x3ff) ||
                    (((client->flags & I2C_M_TEN) == 0) && arg > 0x7f))
                        return -EINVAL;
                if (cmd == I2C_SLAVE && i2cdev_check_addr(client->adapter, 
    arg))
                 {
               ...
               printk(KERN_ALERT "ioctl, client address = 0x%02x",client->addr); 
                client->addr = arg;
                return 0;
        case I2C_TENBIT:
                if (arg)
...
static int i2cdev_open(struct inode *inode, struct file *file)
{
        ...
        //routine_param = kmalloc(sizeof(struct attr_t), GFP_KERNEL); 
out:
        unlock_kernel();
        return ret;
}
...
static int __init i2c_dev_init(void)
{
        ...
        /* Creation of a Work queue */ 
         ...
         p_instance->p_fetched_data = kmalloc(MAX_I2C_TX_RX, GFP_KERNEL); 
