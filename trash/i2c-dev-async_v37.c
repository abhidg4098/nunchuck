 ...
 */
 ...
 /*structure to contain parameters to be sent to work routine */
 struct attr_t
 {
 ...
 struct async_t *p_instance;
 struct attr_t *routine_param;
 
 static void async_routine(void *);  /* Work thread routine */
 int manage_rx(struct attr_t *, union i2c_smbus_data *, int); /* requirement function */
 ...
 static void async_routine(void *routine_param) 
 {
   struct attr_t *temp = (struct attr_t*)routine_param;
   struct i2c_smbus_ioctl_data data_arg = temp->p_arg; 
   struct i2c_client p_client = temp->p_client;
   p_instance->data_status = WORK_QUEUE_BUSY;        
   
   temp->ret_val = i2c_smbus_xfer(p_client.adapter,p_client.addr, p_client.flags,
                   data_arg.read_write, data_arg.command, data_arg.size,
                    temp->p_data);
    
    if (!temp->ret_val && ((data_arg.size == I2C_SMBUS_PROC_CALL) ||
                 (data_arg.size == I2C_SMBUS_BLOCK_PROC_CALL) ||
                 (data_arg.read_write == I2C_SMBUS_READ))) 
     {
       p_instance->p_fetched_data = temp->p_data; // keep the data in kernel for now
 ...
 int manage_rx(struct attr_t *params, union i2c_smbus_data *user_buff, int data_size)
 {
   int ret_val = -WORK_QUEUE_EMPTY;
   if(DATA_READY == p_instance->data_status)
    {
     /*Data was ready and the work was complete, queue should be empty now */
      if (copy_to_user(user_buff, p_instance->p_fetched_data, data_size))
        return -EFAULT;
  ...
  if( 0 != queue_work(async_queue, launch_work))
   {
                 ...
                 unsigned long arg)
 {
     ...
     //abhi
      routine_param->p_client = *client; 
      routine_param->p_arg = data_arg;
      routine_param->ret_val = 0;
      //implementing the work queue

      res = manage_rx(routine_param, &temp, datasize); //(func_params, user_buff, size)
...
/* ------------------------------------------------------------------------- */
 ...
 static int __init i2c_dev_init(void)
 {
         ...
         launch_work = p_instance->async_work;
         INIT_WORK(launch_work, async_routine);
